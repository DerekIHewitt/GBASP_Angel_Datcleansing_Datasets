...
=============================================*/
CREATE PROCEDURE [DatasetProWat].[Customer_Comm_Method_SendToTables]
--(
...
SET NOCOUNT ON;
	DECLARE @MIG_SITENAME varchar(5) = 'GBASP';
	 ...
	 BEGIN TRANSACTION
		DELETE FROM [Dataset].[Customer_Comm_Method_dc] 
	...
	SELECT
		  ...
		  CASE WHEN RIGHT(REPLACE(TRIM([DatasetProWat].[CleanString](CUS_Email1)),' ',''),1) = ';' THEN 
			   ...
			   ELSE REPLACE(TRIM([DatasetProWat].[CleanString](CUS_Email1)),' ','') END	AS CUS_Email1,
		  CASE WHEN RIGHT(REPLACE(TRIM([DatasetProWat].[CleanString](CUS_Email2)),' ',''),1) = ';' THEN
			   ...
			   ELSE REPLACE(TRIM([DatasetProWat].[CleanString](CUS_Email2)),' ','') END	AS CUS_Email2,
		  ...
		  CASE WHEN RIGHT(REPLACE(TRIM([DatasetProWat].[CleanString](CUS_DMEmail)),' ',''),1) = ';' THEN
			   ...
			   ELSE REPLACE(TRIM([DatasetProWat].[CleanString](CUS_DMEmail)),' ','') END	AS CUS_DMEmail,
		  ...
		  TRIM([DatasetProWat].[CleanString](CUS_HHDNMethod)) AS CUS_HHDNMethod
		INTO		#tmpCustomers
		FROM		DatasetProWat.Syn_Customer_dc			WITH (NOLOCK)		-- view for In-Scope Customers from Extraction DB
	...
	INSERT INTO [Dataset].Customer_Comm_Method_dc  --RJS 2021-11-01 Added specific column list to fix proc error
		(  [MIG_SITE_NAME]
		  ,[MIG_COMMENT]
		  ,[MIG_CREATED_DATE]
		  ,[IDENTITY]
	...
	INSERT INTO [Dataset].Customer_Comm_Method_dc
		(		[MIG_SITE_NAME]
			   ,[PARTY_TYPE_DB]
			   ,[IDENTITY]
			   ,[COMM_ID]
