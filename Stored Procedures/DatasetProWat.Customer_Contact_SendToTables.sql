...
=============================================*/
CREATE PROCEDURE [DatasetProWat].[Customer_Contact_SendToTables]
AS
BEGIN
SET NOCOUNT ON;
	...
	BEGIN TRANSACTION
		DELETE FROM [Dataset].[Customer_Contact_dc] 
	...
	SELECT
		...
		AND (Dataset.Filter_Customer('GBASP', 'dc', ISNULL(Dataset.Customer_Filter_Override.isAlwaysIncluded, 0), ISNULL(Dataset.Customer_Filter_Override.IsAlwaysExcluded, 0), 
	...
	INSERT into	[Dataset].[Customer_Contact_dc] 
		(  [MIG_SITE_NAME]
		  ,[MIG_COMMENT]
		  ,[MIG_CREATED_DATE]
		  ,[CUSTOMER_ID]
