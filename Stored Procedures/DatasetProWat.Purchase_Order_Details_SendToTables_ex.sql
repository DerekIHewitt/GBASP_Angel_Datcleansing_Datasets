...
=============================================*/
CREATE PROCEDURE [DatasetProWat].[Purchase_Order_Details_SendToTables_ex]
AS
BEGIN
SET NOCOUNT ON;
	...
	BEGIN TRANSACTION
		DELETE FROM [Dataset].[Purchase_Order_Details_ex] 
	...
	INSERT into	[Dataset].[Purchase_Order_Details_ex] 
		(  [MIG_SITE_NAME]
      ,[MIG_COMMENT]
      ,[MIG_CREATED_DATE]
      ,[CUSTOMER_ID]
      ...
      ,[PO_EL_EXPIRY]
		...
		replace(DatasetProWat.CONVERTFROMCLARION(CUS_PODATE),'1800-01-01','') AS [CUSTOMER_PO_EXPIRY],
		trim(isnull(CUS_PORENT,''))					AS [PO_RENT],
		9999999						AS [PO_RENT_VALUE],
		'2001-01-01'				AS [PO_RENT_START],
		replace(DatasetProWat.CONVERTFROMCLARION(CUS_PORENTDATE),'1800-01-01','') AS [PO_RENT_EXPIRY],
		trim(isnull(CUS_POSANI,''))					AS [PO_SANI],
		9999999						AS [PO_SANI_VALUE],
		'2001-01-01'				AS [PO_SANI_START],
		replace(DatasetProWat.CONVERTFROMCLARION(CUS_POSANIDATE),'1800-01-01','') AS [PO_SANI_EXPIRY],
		trim(isnull(CUS_POEL,''))					AS [PO_EL],
		9999999						AS [PO_EL_VALUE],
		'2001-01-01'				AS [PO_EL_START],
		replace(DatasetProWat.CONVERTFROMCLARION(CUS_POELDATE),'1800-01-01','') AS [PO_EL_EXPIRY]
