...
SET ANSI_NULLS ON
GO
CREATE VIEW [DatasetProWat].[SerialObject_ex_Filtered]
AS
...
= 'S' THEN 'OUT_OF_OPERATION' ELSE 'IN_OPERATION' END AS NX_OPERATIONAL_STATUS, ISNULL(HAS_PEDAL, '') AS NX_HAS_PEDAL,  Dataset.ConvertProwatDate(eqh_WarEndDate,'') AS NX_WARRANTY_END_DATE, Dataset.Filter_SerialObject('GBASP', 'ex', 
                         ...
                         DatasetProWat.Syn_EQTYPE_ex T_ET ON T_ET.ety_id = T_ST.sto_eqtype LEFT JOIN
                             (SELECT        EQH_IDNO AS PEDAL_ID, REPLACE(REPLACE(SUBSTRING(EQH_ID, 4, 100), ' ', ''), '-', '') AS SERIALLINK, '1' AS HAS_PEDAL, STO_Description AS pedal_name
                               FROM            DatasetProWat.Syn_EquipHdr_ex EQ JOIN
                                                         DatasetProWat.Syn_Customer_ex C ON C.CUS_ACCOUNT = EQ.EQH_ACCOUNT JOIN
                                                         DatasetProWat.Syn_Stock_ex ST ON st.sto_stock_code = eq.eqh_stock_code JOIN
                                                         DatasetProWat.Syn_EQTYPE_ex ET ON et.ety_id = st.sto_eqtype
                               WHERE        et.ety_name LIKE 'PEDAL') PDL ON PDL.SERIALLINK = T_EH.EQH_ID
WHERE        (Dataset.Filter_SerialObject('GBASP', 'ex', ISNULL(Dataset.SerialObject_Filter_Override.IsAlwaysIncluded, 0), ISNULL(Dataset.SerialObject_Filter_Override.IsAlwaysExcluded, 0), 
...
Begin DesignProperties = 
   ...
   End
End
', 'SCHEMA', N'DatasetProWat', 'VIEW', N'SerialObject_ex_Filtered', NULL, NULL
GO
DECLARE @xp int
SELECT @xp=1
EXEC sp_addextendedproperty N'MS_DiagramPaneCount', @xp, 'SCHEMA', N'DatasetProWat', 'VIEW', N'SerialObject_ex_Filtered', NULL, NULL
GO
