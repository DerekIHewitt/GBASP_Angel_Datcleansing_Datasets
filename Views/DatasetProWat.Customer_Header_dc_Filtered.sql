...
'TRUE' AS EMAIL_INVOICE_DB, CASE WHEN ISNULL(TRIM(CONVERT(nvarchar(100), dc.CUS_AcqFrom)), '') IN ('62','64') THEN 'B' ELSE ISNULL(CONVERT(VARCHAR(10), dc.CUS_SectorID), '{NULL}') END AS CRM_ACCOUNT_TYPE, dc.CUS_PayType AS PAYMENT_METHOD, '' AS PAY_ADDR_DESCRIPTION, ISNULL(CONVERT(VARCHAR(50), 
...
CASE WHEN dc.cus_Invtype = 'O' THEN CASE WHEN dc.cus_invcons = 0 THEN 'O' ELSE 'P' END ELSE dc.cus_Invtype END AS NX_PAY_OUTPUT_MEDIA_DB, 'FALSE' AS NX_DEFAULT_PAYMENT_METHOD, 
CASE WHEN dc.CUS_TYPE LIKE ('%TRIAL%') THEN 'YES' ELSE 'NO' END AS NX_ACTIVE_TRIAL_DB, CASE WHEN dc.Cus_Industry = '9' THEN 'TRUE' ELSE '' END AS NX_CCA_FLAG_DB, ISNULL(LEFT(TRIM(dc.CUS_CustOrder), 99), '') 
AS NX_BLANKET_PURCHASE_ORDER, 'FALSE' AS NX_CREDIT_BLOCK, CASE WHEN ISNULL(dc.CUS_PODate, 0) = 0 THEN '' ELSE CASE WHEN ISNULL(LEFT(TRIM(dc.CUS_CustOrder), 99), '') = '' THEN '' ELSE CONVERT(nvarchar(21), 
Dataset.ConvertProwatDate(dc.CUS_PODate, NULL), 126) END END AS NX_PO_EXPIRY_DATE, CASE WHEN ISNULL(LEFT(TRIM(dc.CUS_CustOrder), 99), '') = '' THEN 0 ELSE 9999999 END AS NX_PO_EXPIRY_VALUE, 
0 AS NX_PO_VALUE_USED, CASE WHEN dc.CUS_SectorID = 1 THEN 'NATIONAL' ELSE 'ALL' END AS NX_MESSAGE_GROUP, 0 AS NX_ALLOWED_OVERDUE_AMOUNT, 0 AS NX_ALLOWED_OVERDUE_DAYS, 
...
'' AS NX_CONSOLIDATION_DAY_DB, TRIM(dc.CUS_SageCode) AS NX_SAGE_CODE, TRIM(CONVERT(VARCHAR(100), dc.CUS_Account)) AS NX_OLD_CUST_REF1, ISNULL(TRIM(dc.CUS_ImportAC), '') AS NX_OLD_CUST_REF2, 
ISNULL(TRIM(CONVERT(nvarchar(20), dc.CUS_BookIn)), 0) AS NX_BOOK_IN_DB, '' AS NX_NO_LIMIT_DB, '' AS NX_ON_SITE_RA_DB, CASE WHEN ISNULL(trim(CONVERT(nvarchar(20), dc.CUS_POReq)), 0) 
= 0 THEN 0 ELSE CASE WHEN ISNULL(LEFT(TRIM(dc.CUS_CustOrder), 99), '') = '' AND ISNULL(LEFT(TRIM(dc.CUS_PORent), 99), '') = '' AND ISNULL(LEFT(TRIM(dc.CUS_POSANI), 99), '') = '' AND ISNULL(LEFT(TRIM(dc.CUS_POEL), 99), '') 
