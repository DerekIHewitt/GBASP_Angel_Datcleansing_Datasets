...
CREATE VIEW [DatasetGbaqp].[ServiceContract_Raw_Processed_ex_Filtered]
AS
SELECT        1 AS [ID], 'GBAQP' AS MIG_SITE_NAME, '' AS CUSTOMER_INVOICE, V.MIG_Customer AS CUSTOMER_DELIVERY, LEFT(ISNULL(C.IFS_CUSTOMER_NAME, N'No Mapping'), 40) AS CONTRACT_NAME, 
...
FROM            dbo.SC_DATA_ROANNE_GBAQP AS V LEFT OUTER JOIN
                         NEXUS_Transform.dbo.MAP_Customer AS C ON C.MIG_SITE_NAME = 'GBAQP' AND C.LEGACY_CUSTOMER_REF = V.MIG_Customer 
						 --LEFT OUTER JOIN
                         --dbo.CARIAD_PM_DATA_LOAD_FILE AS P ON P.MACHINE_LEGACY_UID = V.MIG_Object 
						 LEFT OUTER JOIN
                         dbo.SC_DATA_ROANNE_GBAQP AS EL ON EL.MIG_Object = V.MIG_Object AND EL.MIG_NISP_Type = 'EL'
WHERE        (V.MIG_NISP_Type <> 'EL')
UNION
SELECT        1 AS [ID], 'GBAQP' AS MIG_SITE_NAME, '' AS CUSTOMER_INVOICE, V.MIG_Customer AS CUSTOMER_DELIVERY, LEFT(ISNULL(C.IFS_CUSTOMER_NAME, N'No Mapping'), 40) AS CONTRACT_NAME, '6' AS MATRIX_TYPE, 
                         '' AS SITE, 'Month' AS INTERVAL_UNIT, 'Prior' AS NX_INVOICE_RULE, V.MIG_Line_Date AS INVOICE_START_DATE, V.MIG_SER_Date AS NEXT_PM_DATE, 'A' AS NX_SERVICE_NEXT_PM_BASE, CAST(NULL AS DATE) AS PO_VALID_FROM, 
...
Begin DesignProperties = 
   ...
   End
End
', 'SCHEMA', N'DatasetGbaqp', 'VIEW', N'ServiceContract_Raw_Processed_ex_Filtered', NULL, NULL
GO
DECLARE @xp int
SELECT @xp=1
EXEC sp_addextendedproperty N'MS_DiagramPaneCount', @xp, 'SCHEMA', N'DatasetGbaqp', 'VIEW', N'ServiceContract_Raw_Processed_ex_Filtered', NULL, NULL
GO
