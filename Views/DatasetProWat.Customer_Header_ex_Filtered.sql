...
ISNULL(CASE WHEN ex.cus_acct_to_inv = 0 THEN '' ELSE TRIM(CONVERT(VARCHAR(100), ex.cus_acct_to_inv)) END, '') AS INVOICE_CUSTOMER,
'' AS INVOICE_SORT_DB, 
'FALSE' AS EMAIL_ORDER_CONF_DB, 
'TRUE' AS EMAIL_INVOICE_DB, 
CASE WHEN ISNULL(TRIM(CONVERT(nvarchar(100), ex.CUS_AcqFrom)), '') IN ('62','64') THEN 'B' ELSE ISNULL(CONVERT(VARCHAR(10), ex.CUS_SectorID), '{NULL}') END AS CRM_ACCOUNT_TYPE,
ISNULL(invoice_customer_info.CUS_PayType,'') AS PAYMENT_METHOD, '' AS PAY_ADDR_DESCRIPTION, 
...
'0' AS NX_MARKET_CODE, 'DIR' AS NX_ORDER_TYPE, '' AS NX_IDENTITY_TYPE_DB, 
...
CASE WHEN ex.CUS_TYPE LIKE ('%TRIAL%') THEN 'YES' ELSE 'NO' END AS NX_ACTIVE_TRIAL_DB, CASE WHEN ex.Cus_Industry = '9' THEN 'TRUE' ELSE '' END AS NX_CCA_FLAG_DB, ISNULL(LEFT(TRIM(ex.CUS_CustOrder), 99), '') 
