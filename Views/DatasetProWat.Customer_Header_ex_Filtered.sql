...
format(ex.CUS_ACNumber,'00000000')), '') AS PAY_ADDR_ACCOUNT, ISNULL(CONVERT(VARCHAR(100), format(ex.CUS_ACSortCode,'000000')), '') AS PAY_ADDR_SORT_CODE, ISNULL(TRIM(ex.CUS_ACName), '') AS PAY_ADDR_ACC_NAME, '' AS PAY_ADDR_BUILD_SOC_REF,
 '' AS PAY_ADDR_TRANS_CODE, '' AS PAY_ADDR_OUR_REF, 'CA' AS CREDIT_ANALYST, ISNULL(TRIM(CONVERT(nvarchar(100), ex.CUS_AcqFrom)), '') AS ACQUIRED_FROM_COMP, '' AS ASSOCIATION_NO, '0' AS NX_GROUP_ID, 
ex.CUS_Charge_VAT AS NX_TAX_CODE, '9999' AS NX_CUST_GRP, '0' AS NX_MARKET_CODE, 'DIR' AS NX_ORDER_TYPE, '' AS NX_IDENTITY_TYPE_DB, 
