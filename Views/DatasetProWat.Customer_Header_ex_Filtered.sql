...
= '' THEN CASE WHEN CUS_Acct_To_Inv = 0 THEN CASE WHEN Cus_Balance > 0 THEN ISNULL(trim(CONVERT(nvarchar(20), CUS_POReq)), 0) ELSE 0 END ELSE CASE WHEN
    (SELECT        Cus_Balance
      ...
      WHERE        v.CUS_Account = ex.CUS_Acct_To_Inv) > 0 THEN ISNULL(trim(CONVERT(nvarchar(20), CUS_POReq)), 0) ELSE 0 END END ELSE ISNULL(trim(CONVERT(nvarchar(20), CUS_POReq)), 0) 
...
'' AS NX_PRINT_AMOUNTS_INCL_TAX_DB, '' AS NX_CREDIT_CONTROL_GROUP_ID, '' AS NX_CUST_PRICE_GROUP_ID, Dataset.Filter_Customer('GBASP', 'ex', ISNULL(Dataset.Customer_Filter_Override.isAlwaysIncluded, 0), 
...
AS NX_FILTER_STATUS
FROM            DatasetProWat.Syn_Customer_ex ex LEFT OUTER JOIN
