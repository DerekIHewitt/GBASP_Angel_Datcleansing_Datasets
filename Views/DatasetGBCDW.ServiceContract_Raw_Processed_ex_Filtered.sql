...
CREATE VIEW [DatasetGBCDW].[ServiceContract_Raw_Processed_ex_Filtered]
AS
SELECT        1 AS [ID], 'GBCDW' AS MIG_SITE_NAME, '' AS CUSTOMER_INVOICE, V.MIG_Customer AS CUSTOMER_DELIVERY, LEFT(ISNULL(C.IFS_CUSTOMER_NAME, N'No Mapping'), 40) AS CONTRACT_NAME, 
                         CASE V.[MIG_NISP_Type] WHEN 'RS' THEN '1' WHEN 'RO' THEN CASE  WHEN V.[MIG_Object] LIKE 'BIF%' THEN 'B' ELSE 'A' END WHEN 'SM' THEN '3' WHEN 'SO' THEN '4' WHEN 'ROSM-RO' THEN '2' WHEN 'ROSM-SM' THEN '3' WHEN 'SA' THEN '9' WHEN '9999' THEN 'D' END AS MATRIX_TYPE,
                        ...
                        ISNULL(V.[PO_EXPIRY_DATE], CAST(NULL AS DATE)) AS PO_EXPIRY_DATE, ISNULL(V.[PO_NUMBER],'') AS PO_NUMBER, 999999.99 AS PO_VALUE, CAST(NULL AS DATE) AS NX_FIRST_REVALUATION_DATE, '' AS NX_REVALUATION_TYPE, V.MIG_Part AS PART_NO, V.MIG_Price AS PRICE, 
                         1 AS NX_UNIT_QTY, V.MIG_Object AS [LEGACY_MACHINE_ID], CAST(12 / V.MIG_IP_Freq AS INT) AS PERIOD_PRICE_INV_PER_YEAR, CAST(ISNULL(ISNULL(V.MIG_SER_Freq, 6),0) AS INT) AS PM_SERVICE_INTERVAL, 
                         ...
                         CASE V.[MIG_NISP_Type] WHEN 'RS' THEN 'GB:LEVY' WHEN 'RO' THEN CASE  WHEN V.[MIG_Object] LIKE 'BIF%' THEN CASE WHEN ISNULL(EL.MIG_Price, 0) * CAST(ISNULL(12 / EL.MIG_IP_Freq, 0) AS INT) > 0 THEN 'GB:LEVY' ELSE '' END  ELSE 'GB:LEVY' END WHEN 'SM' THEN 'GB:LEVY' WHEN 'SO' THEN 'GB:LEVY' WHEN 'ROSM-RO' THEN 'GB:LEVY' WHEN 'ROSM-SM' THEN '' WHEN 'SA' THEN '' WHEN
...
FROM            dbo.SC_DATA_ROANNE_GBCDW AS V LEFT OUTER JOIN
                         NEXUS_Transform.dbo.MAP_Customer AS C ON C.MIG_SITE_NAME = 'GBCDW' AND C.LEGACY_CUSTOMER_REF = V.MIG_Customer 
						 --LEFT OUTER JOIN
                         --dbo.CARIAD_PM_DATA_LOAD_FILE AS P ON P.MACHINE_LEGACY_UID = V.MIG_Object 
						 LEFT OUTER JOIN
                         dbo.SC_DATA_ROANNE_GBCDW AS EL ON EL.MIG_Object = V.MIG_Object AND EL.MIG_NISP_Type = 'EL'
WHERE        (V.MIG_NISP_Type <> 'EL')
UNION
SELECT        1 AS [ID], 'GBCDW' AS MIG_SITE_NAME, '' AS CUSTOMER_INVOICE, V.MIG_Customer AS CUSTOMER_DELIVERY, LEFT(ISNULL(C.IFS_CUSTOMER_NAME, N'No Mapping'), 40) AS CONTRACT_NAME, '6' AS MATRIX_TYPE, 
                         '' AS SITE, 'Month' AS INTERVAL_UNIT, 'Prior' AS NX_INVOICE_RULE, V.MIG_Line_Date AS INVOICE_START_DATE, V.MIG_SER_Date AS NEXT_PM_DATE, 'A' AS NX_SERVICE_NEXT_PM_BASE, CAST(NULL AS DATE) AS PO_VALID_FROM, 
                         ISNULL(V.[PO_EXPIRY_DATE], CAST(NULL AS DATE)) AS PO_EXPIRY_DATE, ISNULL(V.[PO_NUMBER],'') AS PO_NUMBER, 999999.99 AS PO_VALUE, CAST(NULL AS DATE) AS NX_FIRST_REVALUATION_DATE, '' AS NX_REVALUATION_TYPE, V.MIG_Part AS PART_NO, ISNULL(EL.MIG_Price, 0) 
                         AS PRICE, 1 AS NX_UNIT_QTY, V.MIG_Object AS [LEGACY_MACHINE_ID], CAST(12 / V.MIG_IP_Freq AS INT) AS PERIOD_PRICE_INV_PER_YEAR, CAST(ISNULL(isnull(V.MIG_SER_Freq, 6),0) AS INT) AS PM_SERVICE_INTERVAL, 
                         '' AS CURRENT_CONTRACT, '' AS CURRENT_CONTRACT_NUMBER, CAST(NULL AS DATE) AS CURRENT_CONTRACT_START_DATE, '' AS ORIGINAL_CONTRACT_NUMBER, CAST(NULL AS DATE) 
...
Begin DesignProperties = 
   ...
   End
End
', 'SCHEMA', N'DatasetGBCDW', 'VIEW', N'ServiceContract_Raw_Processed_ex_Filtered', NULL, NULL
GO
DECLARE @xp int
SELECT @xp=1
EXEC sp_addextendedproperty N'MS_DiagramPaneCount', @xp, 'SCHEMA', N'DatasetGBCDW', 'VIEW', N'ServiceContract_Raw_Processed_ex_Filtered', NULL, NULL
GO
