...
SELECT        'GBASP' AS MIG_SITE_NAME, '' AS MIG_COMMENT, getdate() AS MIG_CREATED_DATE, TRIM(CONVERT(varchar(20), CUS_Account)) AS CUSTOMER_ID, Dataset.[ConvertMaxLenCleanString](CUS_Company, '', 100) AS [NAME], 
                         ...
                         ROW_NUMBER() OVER (ORDER BY CUS_Account) AS ID, Dataset.Filter_Customer('GBASP', 'dc', ISNULL(Dataset.Customer_Filter_Override.isAlwaysIncluded, 0), ISNULL(Dataset.Customer_Filter_Override.IsAlwaysExcluded, 0), 
