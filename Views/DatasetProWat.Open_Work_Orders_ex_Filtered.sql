...
AS
--REQUESTED TO INCLUDE ONLINE ORDER AS AN OPEN ORDER TYPE. NEED TO BE GIVEN THE MAPPING VALUE FOR WHICH WORK TYPE IT FALLS UNDER (PUT AS DEL FOR NOW)
--ADDING THE ACTION ORR DESCRIPTION IN THE FAULT DESC AS THE ACTION DOESNT POPULATE IN IFS (ACTION ID IS A SET OF FIVE OPTIONS THAT DOES NOT CORRESPOND TO WHAT WE PULL)
...
,CONCAT(
		'"'+'Legacy Wo No: '
		   +convert(varchar,o.ord_ordernum) 
		   +' ' 
		   +'Order Reason: '
		   + ORR_Description
		   +' '
		   +'Instructions: '
		   +ISNULL(trim(O.ORD_Instruct1),'')
		   +' ',ISNULL(trim(O.ORD_Instruct2),'')
		   +' ',ISNULL(trim(O.ORD_Instruct3),'')
		   + ' ',ISNULL(trim(O.ORD_Instruct4),'')
		   +' ',ISNULL(trim(O.ORD_Instruct5),'')
		   +' ', 'GEN_MISC: '
		   +ISNULL(OG.GEN_MISC,'')
		   +'"'
	    )							AS FAULT_DESC
,CASE 
	WHEN O.ORD_OrdReason IN (4,35,91,107,123) 
	THEN ISNULL(dbo.ConvertFromClarion(QLG_CollectDate),DBO.CONVERTFROMCLARION(ORD_REQUEST_DATE)) 
	ELSE DBO.CONVERTFROMCLARION(ORD_REQUEST_DATE) 
 END								AS START_DATE
,''									AS PLAN_FINISH
,''									AS PLAN_HRS
,'*'								AS PREPARED_BY
...
,CASE 
	...
	ELSE cast(o.ORD_ORDREASON as varchar (16)) 
	END								AS WORK_TYPE  --will need to link to LOV that holds the work types relative to the order reason
,ORR_DESCRIPTION					AS ACTION     --will need to refer to the action types relative to the work types
,CASE 
	WHEN OL.ORL_IDNO != 0 
	THEN OL.ORL_IDNO
    ELSE CASE 
			WHEN (SELECT MAX(ORL_IDNO) FROM [DatasetProWat].[Syn_OrdLines_ex] x WHERE X.ORL_OrderNum = O.ORD_OrderNum ) != 0
...
,--CASE WHEN o.ORD_OrdReason = 135 AND ol.ORL_StockCode =  'GEN_INRAIL' THEN 'INSRAIL-POU' 
 ...
 --	ELSE '' END
 CASE 
	WHEN o.ORD_OrdReason = 135 
	THEN
         CASE 
			WHEN EXISTS (SELECT 1 FROM [DatasetProWat].[Syn_OrdLines_ex] x WHERE X.ORL_OrderNum = O.ORD_OrderNum AND X.ORL_StockCode = 'GEN_INRAIL' )
			THEN 'INSRAIL-POU'
			ELSE 'IMP-POU'
		 END
	WHEN o.ORD_OrdReason in (136,137)					--For Adhoc add 'IMP-POU' 18/03/2022 Email
	THEN 'IMP-POU'
	ELSE '' 
 END								AS STANDARD_JOB_ID	
...
,CASE 
	...
	THEN 'TRUE'
    ELSE 'FALSE' 
 END								AS BOOK_IN_REQUIRED --RYFE 08/03/2022
--, eq.eqh_status_flag
...
LEFT OUTER JOIN (
				 SELECT SUM(orl_price)  price , ORL_OrderNum , ORL_IDNo 
				 FROM DatasetProWat.Syn_OrdLines_ex 
				 GROUP BY ORL_OrderNum,ORL_IDNo
				 ) AS OP 
ON OP.ORL_OrderNum = O.ORD_OrderNum AND OP.ORL_IDNo = OL.ORL_IDNo
LEFT OUTER JOIN (
				 SELECT ORL_OrderNum , STRING_AGG(CAST(TRIM(ORL_DETAILS) AS NVARCHAR(MAX)),':') AS GEN_MISC 
				 FROM DatasetProWat.Syn_OrdLines_ex
				 WHERE orl_stockcode = 'GEN_MISC'
				 GROUP BY ORL_OrderNum
				 ) AS OG 
ON OG.ORL_OrderNum = O.ORD_OrderNum
LEFT JOIN [DatasetProWat].[Syn_QuitItem_ex]  QI   
ON QI.QUI_OrderNum = ol.ORL_OrderNum
AND qi.qui_eqhid = ol.orl_idno
LEFT JOIN [DatasetProWat].[Syn_QuitLog_ex]   QLG  
ON QI.QUI_QuitID = QLG.QLG_id
LEFT OUTER JOIN DatasetProWat.Syn_Customer_ex AS C 
ON C.CUS_Account = O.ORD_Account 
LEFT OUTER JOIN Dataset.Customer_Filter_Override 
ON 'GBASP' = Dataset.Customer_Filter_Override.MIG_SITE_NAME 
AND TRIM(CONVERT(VARCHAR(100), C.CUS_Account)) = Dataset.Customer_Filter_Override.CUSTOMER_ID
...
AND ORD_OrdReason not in (5,6,11,14,104,109,114,116,120,125,130,39) --Added 39 & 5
...
140,							   --Addition of Online Order (DEL work type RS 30/03/2022)
--5,--NO LONGER A CMA NOW A SWP---------------------------------------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
108,115,124,131,132,133,134,     --CMA    worktypes -- Added 134 according to Liam doc Removed 39 according to email 07/03/2022				   
...
AND (Dataset.Filter_Customer('GBASP', 'ex', ISNULL(Dataset.Customer_Filter_Override.isAlwaysIncluded, 0), ISNULL(Dataset.Customer_Filter_Override.IsAlwaysExcluded, 0), ISNULL(Dataset.Customer_Filter_Override.IsOnSubSetList, 0), TRIM(CONVERT(varchar(100), 
