...
SELECT
              ...
              , ISNULL(invoice_customer_info.CUS_PayTerms, 0)               AS PAY_TERM_ID
                                ...
                                (
                                                CASE
                                                                WHEN ex.cus_acct_to_inv = 0
                                                                                THEN ''
                                                                                ELSE TRIM(CONVERT(VARCHAR(100), ex.cus_acct_to_inv))
                                                END, ''
                                )
                        AS INVOICE_CUSTOMER
              , ''      AS INVOICE_SORT_DB
              ...
              , CASE
                                WHEN ISNULL(TRIM(CONVERT(nvarchar(100), ex.CUS_AcqFrom)), '') IN ('62'
                                                                                                , '64')
                                                THEN 'B'
                                                ELSE ISNULL(CONVERT(VARCHAR(10), ex.CUS_SectorID), '{NULL}')
                END                                                                                       AS CRM_ACCOUNT_TYPE
              , ISNULL(invoice_customer_info.CUS_PayType, '')                                             AS PAYMENT_METHOD
              , ''                                                                                        AS PAY_ADDR_DESCRIPTION
              , ISNULL(CONVERT(VARCHAR(50), format(invoice_customer_info.CUS_ACNumber, '00000000')), '')  AS PAY_ADDR_ACCOUNT
              , ISNULL(CONVERT(VARCHAR(100), format(invoice_customer_info.CUS_ACSortCode, '000000')), '') AS PAY_ADDR_SORT_CODE
              , ISNULL(TRIM(invoice_customer_info.CUS_ACName), '')                                        AS PAY_ADDR_ACC_NAME
              ...
              , CASE
                                WHEN ex.cus_Invtype = 'O'
                                                THEN
                                                CASE
                                                                WHEN ex.cus_invcons = 0
                                                                                THEN 'O'
                                                                                ELSE 'P'
                                                END
                                                ELSE ex.cus_Invtype
              ...
              , CASE
                                WHEN ex.CUS_TYPE LIKE ('%TRIAL%')
                                                THEN 'YES'
                                                ELSE 'NO'
                END AS NX_ACTIVE_TRIAL_DB
              , CASE
                                WHEN ex.Cus_Industry = '9'
                                                THEN 'TRUE'
                                                ELSE ''
              ...
              , CASE
                                WHEN ISNULL(ex.CUS_PODate, 0) = 0
                                                THEN ''
                                                ELSE
                                                CASE
                                                                WHEN ISNULL(LEFT(TRIM(ex.CUS_CustOrder), 99), '') = ''
                                                                                THEN ''
                                                                                ELSE CONVERT(nvarchar(21), Dataset.ConvertProwatDate(ex.CUS_PODate, NULL), 126)
                                                END
              ...
              ,
                ...
                CASE
                                WHEN ISNULL(trim(CONVERT(nvarchar(20), ex.CUS_POReq)), 0) = 0
                                                THEN 0
                                                ELSE
                                                /*when po req is false leave as false*/
                                                CASE
                                                                WHEN ISNULL(trim(CONVERT(nvarchar(20), ex.CUS_POReq)), 0)                                         = 1
                                                                                AND ISNULL(LEFT(TRIM(NEXUS_TRANSFORM.DBO.CLEANSTRING(ex.CUS_CustOrder)), 99), '') = ''
                                                                                AND ISNULL(LEFT(TRIM(NEXUS_TRANSFORM.DBO.CLEANSTRING(ex.CUS_PORent)), 99), '')    = ''
                                                                                AND ISNULL(LEFT(TRIM(NEXUS_TRANSFORM.DBO.CLEANSTRING(ex.CUS_POSANI)), 99), '')    = ''
                                                                                AND ISNULL(LEFT(TRIM(NEXUS_TRANSFORM.DBO.CLEANSTRING(ex.CUS_POEL)), 99), '')      = ''
                                                                                AND ISNULL(ex.CUS_Acct_To_Inv, 0)                                                 = 0
                                                                                AND ISNULL(ex.Cus_Balance, 0)                                                     > 0
                                                                                THEN ISNULL(trim(CONVERT(nvarchar(20), ex.CUS_POReq)), 0)
                                                                WHEN ISNULL(trim(CONVERT(                nvarchar(20), ex.CUS_POReq)), 0)                         = 1
                                                                                AND ISNULL(LEFT(TRIM(NEXUS_TRANSFORM.DBO.CLEANSTRING(ex.CUS_CustOrder)), 99), '') = ''
                                                                                AND ISNULL(LEFT(TRIM(NEXUS_TRANSFORM.DBO.CLEANSTRING(ex.CUS_PORent)), 99), '')    = ''
                                                                                AND ISNULL(LEFT(TRIM(NEXUS_TRANSFORM.DBO.CLEANSTRING(ex.CUS_POSANI)), 99), '')    = ''
                                                                ...
                                                                WHEN ISNULL(trim(CONVERT(nvarchar(20), ex.CUS_POReq)), 0)                                         = 1
                                                                                ...
                                                                                (
                                                                                       SELECT
                                                                                              ISNULL(Cus_Balance, 0)
                                                                                              /*take the balance */
                                                                                       FROM
                                                                                              DatasetProWat.Syn_Customer_ex v
                                                                                              /*from customer*/
                                                                                       WHERE
                                                                                              v.CUS_Account = ex.CUS_Acct_To_Inv
                                                                                )
                                                                                > 0
                                                                                THEN ISNULL(trim(CONVERT(nvarchar(20), ex.CUS_POReq)), 0)
                                                                WHEN ISNULL(trim(CONVERT(                nvarchar(20), ex.CUS_POReq)), 0)                         = 1
                                                                                ...
                                                                                <= 0
                                                                                THEN 0
                                                                                ELSE ISNULL(trim(CONVERT(nvarchar(20), ex.CUS_POReq)), 0)
                                                END
                                                /*THEN     --when all po fields are empty
                                                ...
                                                END ELSE ISNULL(trim(CONVERT(nvarchar(20), ex.CUS_POReq)), 0) --take the po as is as the fallback END */
                END                                                    AS NX_PURCHASE_ORDER_REQ_DB
              , ''                                                     AS NX_SHORT_TERM_DB
              , ''                                                     AS NX_COST_CODE
              , ISNULL(trim(CONVERT(nvarchar(20), ex.CUS_RepNum)), '') AS NX_MAIN_REP
              , CASE
                                WHEN ex.CUS_PaperInvCharge != 1
                                                THEN 'TRUE'
                                                ELSE ''
              ...
              , CASE
                                WHEN ex.cus_acct_to_inv                                         != 0
                                                AND isnull(invoice_customer_info.cus_invcons, 0) = 1
                                                THEN 'ALL'
                                ...
                                WHEN isnull(invoice_customer_info.cus_inveachdel, 0) = 0
                                                ...
                                                CASE
                                                                ...
                                                                WHEN isnull(ex.cus_acct_to_inv, 0) = 0
                                                                                THEN 'SITE'
                                                END
                                WHEN isnull(invoice_customer_info.cus_inveachdel, 0) = 1
                                                ...
                                                /* 'No Consolidation'*/
                                                ELSE ''
                                                /* This not in RS orignal code but I think required to prevent NULLS being passed out.*/
                END                                                                        AS NX_BR_CONSOLIDATION
              , REPLACE(ISNULL(TRIM(CONVERT(VARCHAR(100), ex.CUS_INDUSTRY)), ''), '0', '') AS NX_LEGAL_ENTITY_DB
              , CASE
                                WHEN ISNULL(TRIM(CONVERT(VARCHAR(100), ex.cus_importac)), '') = '0'
                                                THEN ''
                                                ELSE ISNULL(TRIM(CONVERT(VARCHAR(100), ex.cus_importac)), '')
                END AS NX_IMPORT_ACCOUNT
              , ''  AS NX_ACCOUNT_GROUP
FROM
                DatasetProWat.Syn_Customer_ex ex
                LEFT JOIN
                                DatasetProWat.Syn_Customer_ex AS invoice_customer_info
