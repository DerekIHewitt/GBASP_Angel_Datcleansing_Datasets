...
-- Description:	Filter for Supplier table
--
--	Ver	Who		When		What
--	1	DIH		03/12/2020	Created from a copy of the Customer filter.
--  2   RYFE    13/02/2020  Removed AccountType because unavailable in table
-- =============================================
CREATE   FUNCTION [Dataset].[Filter_Supplier]
(
...
BEGIN
	...
	DECLARE @CurrentMode int = Dataset.Filter_Mode(@MIG_SITE_NAME, @Scope, 'Supplier');
	
	IF @MIG_SITE_NAME = 'GBASP'
		BEGIN
			------ Find records that require dropping ------------------------------------------------
			IF (@CompanyName LIKE 'ZZZ%')
				BEGIN
					SET @Result = @Result_Drop;
				END
			--ELSE
			--	BEGIN
			--		IF (@AccountType LIKE 'CANC%') OR
			--		   (@AccountType LIKE 'STOCK%') 
			--			BEGIN 
			--				SET @Result = @Result_Drop;
			--			END
			--	END
